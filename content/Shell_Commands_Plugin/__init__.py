...
DECIDE_IF_FILE_IS_ALLOWED_FOR_IMPORT = (
	...
	lambda abs_Path: abs_Path.name != "__main__.py",
...
def is_Lazy_Module(instance):
	# print(f">F> is_Lazy_Module?")
	# print(f">>> instance.__dict__.keys() = {instance.__dict__.keys()}?")
	# if '__class__' not in instance.__dict__: return False
	# print(f">>> instance.__class__ = {instance.__class__}")
	# print(f">>> '.Lazy_Module' in str(instance.__class__) = {'.Lazy_Module' in str(instance.__class__)}")
	return ".Lazy_Module" in str(instance.__class__)


def lazy_import(abs_Path):
	# print(f">>> lazy_import of {abs_Path.name}")
	if abs_Path.name == "__init__.py": abs_Path = abs_Path.parent
	if not is_allowed_for_import(abs_Path): return
	# print(f">>> lazily importing: {abs_Path.name}")
	...
	# print(f">>> IS abs_sys_key in sys.modules? {abs_sys_key in sys.modules}")
	if abs_sys_key not in sys.modules:
		# print(f">>> {abs_sys_key} NOT in sys.modules!")
		sys.modules[abs_sys_key] = Lazy_Module(module, abs_Path)
		# print(f">>> sys.modules[{abs_sys_key}].__class__ = {sys.modules[abs_sys_key].__class__}")
	elif not is_Lazy_Module(sys.modules[abs_sys_key]):
		# print(f">>> {abs_sys_key} NOT a Lazy_Module!")
		# print(f">>> sys.modules[{abs_sys_key}]:\n\t>> {sys.modules[abs_sys_key]}")
		sys.modules[abs_sys_key] = Lazy_Module(module, abs_Path)
	# print(f"\t>> sys.modules[{abs_sys_key}] = Lazy_Module(module, abs_Path)")
	if rel_sys_key and rel_sys_key != abs_sys_key:
		if rel_sys_key not in sys.modules:
			sys.modules[rel_sys_key] = sys.modules[abs_sys_key]
			# print(f">>> sys.modules[{abs_sys_key}].__class__ = {sys.modules[abs_sys_key].__class__}")
		elif not is_Lazy_Module(sys.modules[rel_sys_key]):
			# print(f">>> {rel_sys_key} NOT a Lazy_Module!")
			sys.modules[rel_sys_key] = sys.modules[abs_sys_key]
...
def make_module_callable(Lazy_Module):
	abs_Path = Lazy_Module.__file__ / "__call__.py"
	abs_sys_key = get_abs_sys_key_from_Path(abs_Path)
	lazy_import(abs_Path)
	mod = sys.modules[abs_sys_key]
	load_module(mod)
	if "__call__" not in mod.__module_from_spec__.__dict__:
		err_msg  = f"Error in package: '{Lazy_Module.__name__}'"
		err_msg += f"\n>>> Found file: '__call__.py' in package: '{Lazy_Module.__name__}'"
		err_msg += f"\n>>> However no definition of function '__call__' was found inside this file"
		err_msg += f"\n\t>> Please DEFINE the '__call__' function inside of it, or DELETE the file"
		err_msg += f"\n\t>> Absoulute file location:"
		err_msg += f"\n\t\t> {Lazy_Module.__file__ / '__call__.py'}"
		raise TypeError(err_msg)
	foo = mod.__module_from_spec__.__dict__["__call__"]
	Lazy_Module.__module_from_spec__.__dict__["__call__"] = foo

def load_module(Lazy_Module):
	# print(f">F> Is {Lazy_Module.__name__} LOADED?: {Lazy_Module.__is_loaded__}")
	# print(f"\t>> mem location: {hex(id(Lazy_Module))}")
	if Lazy_Module.__is_loaded__: return
	Lazy_Module.__is_loaded__ = True
	abs_Path = Lazy_Module.__file__
	load_submodules(abs_Path)
	load_last_submodule(Lazy_Module)
	if os.path.isdir(Lazy_Module.__file__):
		if "__call__.py" in os.listdir(Lazy_Module.__file__):
			# print(f">>> FOUND a __call__.py file")
			make_module_callable(Lazy_Module)


def load_module_from_abs_Path(abs_Path):
	abs_sys_key = get_abs_sys_key_from_Path(abs_Path)
	lazy_import(abs_Path)
	load_module(sys.modules[abs_sys_key])
...
def get_attr_Lazy_Module(Lazy_Module, attr):
	...
	if attr in Lazy_Module.__dict__.keys():
		...
		key_error = KeyError(attr)
		err_msg  = f">>> Package '{Lazy_Module.__name__}' has no attribute '{attr}'"
		if os.path.isdir(Lazy_Module.__file__):
			err_msg += f"\n>>> This package refers to this FOLDER:"
			err_msg += f"\n\t>> {Lazy_Module.__file__}"
			err_msg += f"\n>>> To add this key to this package: in the refered folder,"
			err_msg += f"\n\t>>> CREATE A FILE called: '{attr}.py'"
		elif os.path.isfile(Lazy_Module.__file__):
			err_msg += f"\n>>> This package refers to this FILE:"
			err_msg += f"\n\t>> {Lazy_Module.__file__}"
			err_msg += f"\n>>> To add this key to this package: in the refered folder,"
			err_msg += f"\n\t>>> DECLARE an object called: '{attr}'"
		key_error.add_note(err_msg)
		raise key_error
...
def call_Lazy_Module(Lazy_Module, *args, **kwargs):
	...
	load_module(Lazy_Module)
	if Lazy_Module.__name__ in Lazy_Module.__module_from_spec__.__dict__:
		foo = Lazy_Module.__module_from_spec__.__dict__[Lazy_Module.__name__]
		return foo(*args, **kwargs)
	elif "__call__" in Lazy_Module.__module_from_spec__.__dict__:
		foo = Lazy_Module.__module_from_spec__.__dict__["__call__"]
		return foo(*args, **kwargs)
	try:
		foo = Lazy_Module.__module_from_spec__
		return foo(*args, **kwargs)
	except TypeError:
		err_msg  = f"module: '{Lazy_Module.__name__}' is not callable"
		if os.path.isdir(Lazy_Module.__file__):
			# print(">>> IS DIR!")
			err_msg += f"\n>>> This module is a FOLDER, located at:"
			err_msg += f"\n\t>> {Lazy_Module.__file__}"
			err_msg += f"\n>>> TO MAKE IT CALLABLE, please create a file '__call__.py' and "
			err_msg += f"inside of it define the '__call__' function."
			err_msg += f"\n>>> Then, when you call this module that function will be invoked"
		elif os.path.isfile(Lazy_Module.__file__):
			# print(">>> IS FILE!")
			err_msg += f"\n>>> This module is a FILE, located at:"
			err_msg += f"\n\t>> {Lazy_Module.__file__}"
			err_msg += f"\n>>> TO MAKE IT CALLABLE: please define inside of the file a "
			err_msg += f"funtion named: '{Lazy_Module.__name__}'"
			err_msg += f"\n>>> Then, when you call this module that function will be invoked"
		raise TypeError(err_msg)
...
def import_up_to_relative():
	# print(f">F> 'import_up_to_relative', RELATIVE_FOLDER:\n\t>>{RELATIVE_FOLDER}")
	if RELATIVE_FOLDER == ROOT_FOLDER: return
	def recursive_function(abs_Path):
		# print(f">F> 'import_up_to_relative', abs_Path:\n\t>>{abs_Path}")
		if abs_Path == ROOT_FOLDER: return
		load_module_from_abs_Path(abs_Path)
...
def setup():
	...
	load_module_from_abs_Path(ROOT_FOLDER)
	# abs_sys_key = get_abs_sys_key_from_Path(RELATIVE_FOLDER)
	# rel_sys_key = get_rel_sys_key_from_Path(RELATIVE_FOLDER)
	# print(f">>> SETUP : abs_sys_key = {abs_sys_key}")
	# print(f">>> SETUP : rel_sys_key = {rel_sys_key}")
...
# print(f">>> RELATIVE_FOLDER = {RELATIVE_FOLDER}")
setup()
