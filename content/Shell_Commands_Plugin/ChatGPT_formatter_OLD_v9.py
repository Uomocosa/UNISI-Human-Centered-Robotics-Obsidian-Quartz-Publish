...
def Test(): 
	tests = [None, Test_1, Test_2, Test_3]
	assert tests[2]()
...
def remove_new_lines_from_multiline_math_formulas(text):
	...
	for i in range(len(splitted_text)-1):
		...
		prev_formula = '$$'+matches[i-1].group(1).lstrip().rstrip()+'$$'
...
def handle_titles(text):
	...
	for i in range(len(matches))[::-1]:
		text_after_title = in_between_splitted_text[i].lstrip()
		if (
			is_bullet_list(text_after_title) or
			is_numeric_list(text_after_title)
		):
			print(f">>> FOUND A LIST AFTER the TITLE: '{italic_bold_titles[i]}'")
			text += italic_bold_titles[i] + '\n' + text_after_title
		else:
			text += italic_bold_titles[i] + '<br>' + text_after_title
